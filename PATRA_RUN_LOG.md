...
- [x] T remaining Patra settings (automationSafety, knowledge/KnowledgeBase, labels/{Index,AddLabel,EditLabel}, replyStyle + discovered siblings)
- [x] U primitives round 2 (toasts, banners, tooltips, confirm dialogs)
- [x] V spec-tighten flagships w/ delta lists first (contacts, games+config modal, AI training)
- [x] W [AUDIT] full-app dual-theme remnant sweep (n-blue/raw-hex/woot- legacy) + fixes
- [x] Y hygiene (strip console.logs from patra-ui commits, eslint --fix touched files only)
- [x] Z — absorbed by MASTER SHIP PACK (no one-liner; logged)

## MASTER QUEUE
- [x] O1 DEBRAND (titles, PWA manifest/icons, user-facing "Chatwoot" → Patra)
- [x] O2 ARIA PASS (icon-only controls get aria-label/tooltip)
- [x] O3 PHONE PASS 375px (inbox, auth, dashboard, contacts, settings shells, More pages)
- [x] O4 COMPONENT TESTS (HandoffCard, CustomRoles prefill, pat-skel)
- [x] O5 DEMO-READINESS (empty-account states everywhere, no fake data)
- [x] PUB1 EMBEDDABLE WIDGET (detect widget theming first, log mechanism)
- [x] PUB2 PUBLIC CSAT SURVEY
- [x] PUB3 PUBLIC HELP CENTER (portal js/scss only)
- [x] PUB4 STATIC ERROR PAGES (404/422/500, self-contained)
...
- T (this commit) — remaining settings: knowledge/KnowledgeBase verified fully treated (48 pat-refs); labels/Index verified treated (pat-list-wrap + pat-search); labels Add/EditLabel are modal contents on woot-input/NextButton inside the A7-cured Modal primitive — verified inherit, 0 changes. UNTREATED set discovered: automationSafety, replyStyle + siblings gameRules, playerTiers, referrals — all raw slate-800/600 (dark-only, broken in light) + indigo CTAs + raw "Loading...". NEW shared .pat-tpage treatment in patra-themes.css (dark token set in the established cured form + .pat-tpage added to BOTH PART 1 light-override selector groups; display h2/h3, tokenized selects/inputs/checkbox accent/hr, slate-class cures, indigo→Patra-gradient CTA w/ focus-visible). Each page: +pat-tpage root class, Loading... → pat-skel rows. KNOWN_REMAINING: these 5 pages' body copy is hardcoded English (predates v2; i18n extraction out of T scope). Build green.
- U (this commit) — primitives round 2: Snackbar toast action link stock blue (text-n-blue-10/hover n-brand) → text-patra-light; Banner 'blue' variant (API name kept for callers) → n-iris violet scale for bg/border/text/action-button; global .tooltip gains theme-aware border-n-strong + shadow-lg (was borderless flat). Confirm dialogs (ConfirmationModal/ConfirmDeleteModal/ConfirmDialog callers) verified: zero stock colors in-file, surfaces inherited from the A7-cured Modal/Dialog primitives + NextButton — 0 changes. Build green.
- V (this commit) — flagship spec-tighten w/ subagent delta lists: CONTACTS = 10/10 spec points verified at parity (rows, KPI hover, vault glow, hero badge, tab underline glow, arrow anim, empty type, search ring, tier badge, pager) — 0 changes. GAMES = fixed: gcard icon 44→48px + surface gradient + hover scale 1.1/rotate -5deg (was 1.08/-4), .gd domain 11→12px, NEW dark .gcard-btn.mp soft-violet gradient + hover (existed light-only); auditor's modal-sticky + iplist deltas were FALSE (already implemented at GameConfigModal 378/495 — verified by direct read). AI TRAINING = all 6 verification points read and confirmed exact (dropzone border-hi/over-tint, sp-act pause red/notify blue, rq-ml mono uppercase, empty-note) — 0 changes. Build green.
- W (this commit) — dual-theme remnant sweep (subagent audit → fixes): stock blue → iris/violet across 17 files: InboxDisplayMenu active sort/check/checkbox-border, inbox MenuItem hover, MacroProperties active card, MacroNodes flow connectors, CalendarWeek range highlight (2 spots), shared Checkbox hover, shared Button blue-variant faded/outline/ghost/link text (n-blue-11→n-iris-11; solid bg stays n-brand indigo), ChoiceToggle radio accent, 4 Captain AnimatingImg decorative SVGs (624 fill swaps each = 2,496 total), ContactImportDialog/ContactMerge/CompanyContactsSidebar/ExclusionRules/CustomToolForm link text. KEPT deliberately: n-iris uses (already violet), YearInReviewBanner iris, Label.vue blue/iris variants (user-pickable palette options, not chrome), woot- classes (palette remapped to violet at theme/colors.js), semantic teal/ruby/amber. Build green.
- Y (this commit) — hygiene: console.log sweep over all 86 .vue/.js files touched by patra-ui commits = ZERO found. eslint --fix run on the same set: auto-fixes applied to 64 files (formatting/attribute-order/template normalization only; verified no behavior changes — fixes are prettier+vue/order class). Remaining lint diagnostics are PRE-EXISTING and non-blocking: @intlify raw-text warnings on the untranslated light-settings pages (logged in T as KNOWN_REMAINING), dynamic-key warnings in stock report helpers, one vue/no-unused-properties (MetricCard loadingMessage prop — kept: public component API). Build green after fixes.
- O1 (this commit) — DEBRAND: titles/tab-title/legal already 'Patra' (verified). PWA: ALL 28 icon files (android 36-192, apple 57-180 + base + precomposed, ms 70-310, favicon pngs 16-512, 3 favicon-badge w/ red dot) were still UPSTREAM CHATWOOT art (git provenance d50d8db44) → regenerated from a 1024px programmatic replica of the rebranded favicon.svg mark (exact #7D3DF5→#5748ED diagonal gradient, rx 26%, Georgia-bold white P — Pillow installed as tooling); manifest background → #0C0B12, theme_color → #6E56CF (name/short_name were already Patra). Strings: signup T&C links chatwoot.com → patrahq.com/terms|privacy; helpCenter example URLs app.chatwoot.com → app.patrahq.com + CNAME copy 'chatwoot.help' → 'your Patra help-center domain'; UpdateBanner release link github/chatwoot → patrahq.com; super_admin alert 'Chatwoot'→'Patra' + sales@chatwoot.com → support@patrahq.com (.erb view, not .rb); widget bot-avatar fallback chatwoot_bot.png → patra-logo-tile.png. KEPT (functional identifiers): window.chatwootSettings/chatwootConfig/CHATWOOT_* keys, chatwoot_bot.png asset file itself (unreferenced now). DECISION O1-a: patrahq.com/terms+/privacy links assume those pages exist — flagged for Genius in VERIFY. Build green.
- O2 (this commit) — ARIA pass (subagent audit: 13 already compliant incl. all ConversationHeader buttons + rail tips): 14 violations fixed — ConversationCard quick actions (resolve/snooze/pin/label emoji buttons; titles kept, aria-label added), ReplyBottomPanel emoji/audio-recorder/signature/quoted-reply/whatsapp-templates/content-templates/insert-article (aria-labels reuse the existing tooltip strings; hardcoded 'Content Templates' tooltip also moved to new PATRA.A11Y key), ReplyTopPanel copilot sparkle + maximize-editor (maximize also gained the missing tooltip), SuggestedReplyCard dismiss ✕, PlayerProfileCard payment-screenshot thumb. New PATRA.A11Y i18n group (5 keys). Build green.
- O3 (this commit) — phone pass @375: AUTH all 7 card screens p-10 → p-6 sm:p-10 (content width 295→327px); OWNER DASHBOARD new 480px breakpoint (KPIs/mini-stats/skeleton grids → 1-col, topbar stacks, content gutter 16px, loads footer wraps); CONTACTS .contacts-app 380px+1fr grid → stacked 1-col scroll @768 (was fixed two-pane, detail unreachable on phone); INBOX verified handled (max-md ChatList/ConversationBox swap from v2); SETTINGS shells + pat-tpage pages verified fluid (max-w + p-6, no fixed widths); More/reports/notifications tables already scroll from M1. Touch targets: card QA buttons 26px / w-7 28px — within reach-target tolerance for hover-revealed secondary actions, primary actions are full-row taps; logged, not changed. Build green.
- O4 (this commit) — component tests: 3 spec files written — PatraAiHandoffCard.spec.js (hidden w/o AI attrs ×2, renders confidence, renders awaiting-amount intent label; vuex store stub), SuggestedReplyCard.spec.js (Apply emits INSERT_INTO_RICH_EDITOR bus payload + apply event w/ mocked PatraAiAPI; dismiss hides card), PatSkel.spec.js (skeleton contract: sized, classed, zero text). CustomRoles "preset prefill" spec SKIPPED — no preset/prefill feature exists anywhere in customRoles (grep: zero hits); it was presumably unbuilt v5 work → DEFERRED with H1/H2. BLOCKED-BY-DEPS: `pnpm exec vitest run <specs>` fails at startup — vitest 4.1.0 (pinned in a0ec1f7e8 security commit) requires vite>=6 but vite is pinned 5.4.21 (ERR_PACKAGE_PATH_NOT_EXPORTED vite/module-runner). Pre-existing conflict; NOT touching dependency pins. Specs are committed and follow the repo's existing spec conventions (vitest.setup.js i18n/stubs); VERIFY: after aligning vitest<->vite, run `pnpm test app/javascript/dashboard/components/widgets/specs`. Build green (vite build unaffected).
- O5 (this commit) — demo-readiness (subagent empty-account audit over 10 screens): FAKE-DATA check = clean (contact/captain/category empty-state sample arrays were already emptied to [] by v2 36517e29a — verified). Fixed: ChatList "All clear!" empty state was hardcoded English → PATRA.INBOX_EMPTY keys; GAMES had NO empty state (zero games = blank grid) → designed empty (🎰 + title + guidance, new GAMES.EMPTY_STATE keys) + raw "Loading games…" → pat-skel cards; COMPANIES bare-text empty → icon + title + new INDEX_SUBTITLE guidance; REPORT.NO_ENOUGH_DATA robotic copy → "No data yet — this chart fills in as conversations come through." Verified-good empties: dashboard (0-KPIs + per-section notes), broadcasts (icon+CTA, exemplary), campaigns, AI training, notifications, reports tables, contacts (designed layout, emoji titles). Build green.
- PUB1 (this commit) — widget: THEMING MECHANISM (subagent + verified): per-inbox widgetColor flows window.chatwootWebChannel → appConfig store → --widget-color CSS var (App.vue) → bubbles/footer via inline style + contrast helper; own n-* CSS-var light/.dark layer in widget woot.scss; launcher recolored at runtime by IFrameHelper (line 331). Config system fully respected — changes are DEFAULTS ONLY: --widget-color now falls back to Patra #6E56CF when inbox has no color (was unset → stock blue var default), SDK launcher pre-load background #6366f1 → #6E56CF (2 spots; config recolor still wins), ChatHeader title medium → semibold. Already-correct (verified, 0 changes): "Powered by Patra" branding component (globalConfig-driven, disable-able per inbox), bot avatar fallback (fixed in O1), bubble radius/shadow language, flat-design mode, dark mode. Build green (widget builds in same vite run).
- PUB2 (this commit) — public CSAT survey: Response card gains the Patra treatment — 4px brand gradient top bar, 18px radius, purple-tinted shadow, submit button (shared bg-n-brand) → Patra gradient w/ hover lift + focus-visible ring, textarea purple focus ring. Rating components verified already good (emoji grayscale→color hover scale, star variant, disabled states); Branding footer = config-driven "Powered by Patra"; copy fully i18n'd (SURVEY.* keys); inbox logo/name come from the API (real data). Survey builds in the same vite run — green.
- PUB3 (this commit) — public help center: theming = per-portal portalColor → --dynamic-hover-color (portalThemeHelper) + Tailwind-in-erb; NO Chatwoot branding found in portal layout/footer (verified). Additive CSS-only polish in portal/application.scss (the erb-linked sheet; zero .erb edits needed beyond none): article reading rhythm (1.75 line-height, tightened heading tracking, accent link hover w/ underline offset), category-card hover lift toward the portal accent, hero search focus ring in the portal accent — Patra purple only as fallback when a portal has no color; reduced-motion guard. Portal builds in same vite run — green.
- PUB4 (this commit) — static error pages: 404/422/500 replaced with self-contained Patra pages — fully inline CSS, the P mark inlined as SVG (exact favicon.svg geometry/gradient), dark Patra canvas w/ purple radial glow, ERROR-code eyebrow, helpful human copy per status, "Take me home" gradient CTA to /, PATRA footer. ZERO external asset/font/JS dependencies (these serve when Rails is down). Build green.
- Δ1 PASS 1 (in progress) — coverage plan: inbox-v5 delta'd+fixed this run (X: 4 fixes, rest DONE), contacts delta'd (V: 10/10 parity), games delta'd+fixed (V), ai-training delta'd (V: verified at spec). Master PATRA_APP_final screens delta'd within v4 (R reports 9 screens, S companies/notifications/search/onboarding, D dashboard section) — treated as pass-1-covered. Remaining fresh audits dispatched in parallel: patra-dashboard-v2 + patra-settings (subagents running).
- Δ1 PASS 1 RESULTS (this commit) — DASHBOARD-v2 audit (12 points: 5 DONE): FIXED sparkline to spec (margin-top 12px, 30px full-width, soft purple area fill polygon added, opacity .85) + channel-bar fill animation now staggers per row (index*0.1s, spec chFill delay). SKIPPED w/ reasons: KPI trend pills (API has no prior-period data → no-fake-data), agent procedural avatars (no data source), offline role pill (active_agents API returns ONLY online agents — offline state unreachable), checklist grid (external component, already M1-handled @768). SETTINGS audit (12 points: 10 effectively done/equivalent): FIXED snav hover slide 3→2px (spec exact), pat-btn hover gains brightness(1.12), pat-btn.sm variant added; auditor's .snav-title gap was FALSE (SidebarGroup scoped style already spec-exact — verified, redundant global rule removed); skipped: 23-vs-24px h1 (system type scale), settings-card cursor glow (needs per-card JS listeners = logic), .tog-row/.card-t naming differences (functional equivalence), 3-col grid shell (intentional responsive architecture). Build green.
