"""
Orion Stars automation — controls AdsPower-protected Chrome via Playwright.
...
ORION_STARS_USER = os.getenv("ORION_STARS_USER", "")
ORION_STARS_PASS = os.getenv("ORION_STARS_PASS", "")
ORION_STARS_URL = os.getenv("ORION_STARS_URL", "https://orionstars.vip:8781/default.aspx")
...
def log(msg: str) -> None:
    ...
    print(f"[orion_stars] {msg}", file=sys.stderr, flush=True)


def dismiss_announcement_popup(page):
    """Close the Rocket Ramp announcement modal if present. Idempotent — safe to call when popup isn't there."""
    try:
        overlay = page.locator("div#alertOverlay.show").first
        if overlay.is_visible(timeout=2000):
            print("[orion_stars] Rocket Ramp popup detected — dismissing")
            # Try the known close button id first
            close_btn = page.locator("#cancelBtn").first
            if close_btn.is_visible(timeout=500):
                close_btn.click(timeout=3000)
            else:
                # Fallback: click any Close button inside the overlay
                page.locator("div#alertOverlay button:has-text('Close')").first.click(timeout=3000)
            # Wait for overlay to actually disappear
            page.wait_for_selector("div#alertOverlay.show", state="hidden", timeout=5000)
            print("[orion_stars] Popup dismissed")
        # No popup visible — silent return
    except Exception as e:
        # Last resort: remove the overlay via JS so it can't block clicks
        try:
            page.evaluate("document.querySelectorAll('#alertOverlay').forEach(el => el.remove())")
            print(f"[orion_stars] Popup force-removed via JS (handler exception: {e})")
        except Exception:
            pass  # Silent — if even JS fails, downstream click will surface the real error
...
def solve_captcha_via_telegram(image_bytes: bytes, request_id: str) -> str:
    ...
    caption = (
        f"🔢 CAPTCHA needed — Orion Stars login\n"
...
def wait_for_topup_signal(player: str, amount: float) -> bool:
    ...
    alert = (
        f"⚠️ <b>AGENT FUNDS NEEDED</b> | Orion Stars\n"
        f"Agent: <code>{ORION_STARS_USER}</code>\n"
    ...
    while time.time() - start < TOPUP_TIMEOUT_SEC:
        ...
        for upd in updates.get("result", []):
            ...
            if "loaded" in text:
                ...
                telegram_send_message(TOPUP_ALERT_GROUP_ID, f"✅ Top-up confirmed — retrying recharge for <code>{player}</code> (agent: <code>{ORION_STARS_USER}</code>)")
                return True
...
def login_orion_stars(page: Page) -> bool:
    ...
    log(f"Navigating to {ORION_STARS_URL}")
    page.goto(ORION_STARS_URL, wait_until="domcontentloaded", timeout=45000)
    page.wait_for_timeout(2000)
    if is_dashboard_visible(page):
    ...
    for attempt in range(1, LOGIN_MAX_RETRIES + 1):
        ...
        try:
            ...
            try:
                ...
                pass
            page.locator("input#txtLoginName").fill(ORION_STARS_USER)
            page.locator("input#txtLoginPass").fill(ORION_STARS_PASS)

            # Screenshot just the CAPTCHA image element
            captcha_bytes = page.locator("img#ImageCheck").screenshot()
            request_id = f"os_{int(time.time())}"
...
def search_player(page: Page, account: str) -> dict | None:
    ...
    for attempt in range(2):
        ...
        page.wait_for_timeout(2000 if attempt else 1500)
...
def _do_action_with_amount(
    ...
    try:
        dlg.locator("#Button1").click(timeout=2000)
    except Exception:
        # Orion Stars: #Button1 has display:none. The visible submit is an <a> postback link inside the dialog.
        dlg.locator("a.btn13.btn-danger1").first.click()
    page.wait_for_timeout(1500)
    ...
    for attempt in range(1, TOPUP_MAX_RETRIES + 1):
        try:
            ...
            if attempt >= TOPUP_MAX_RETRIES:
                raise RuntimeError(
                    f"AGENT_FUNDS_EXHAUSTED: agent {ORION_STARS_USER} still insufficient after {TOPUP_MAX_RETRIES} top-up requests for player {account} amount ${amount:.2f}"
                )
...
def create_player(page: Page, account: str, password: str) -> dict:
    ...
    try:
        dlg.locator("body").evaluate("() => { if (typeof __doPostBack === 'function') { __doPostBack('ctl07',''); } else { throw new Error('no postback'); } }")
    except Exception:
        dlg.locator("#Button1").click()

    page.wait_for_timeout(1000)
    popup_text = ""
    try:
        text, ctx = wait_for_success_popup(page, timeout_ms=5000)
        popup_text = text.strip()
        dismiss_popup(ctx)
    except (TimeoutError, PlaywrightTimeout) as e:
        log(f"No create popup detected — verifying create by search: {e}")
...
def validate_env() -> list[str]:
    missing = []
    for key in ("ADSPOWER_API_KEY", "ORION_STARS_USER", "ORION_STARS_PASS", "TELEGRAM_BOT_TOKEN"):
