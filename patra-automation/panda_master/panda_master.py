"""
Panda Master automation — controls AdsPower-protected Chrome via Playwright.
...
PANDA_MASTER_USER = os.getenv("PANDA_MASTER_USER", "")
PANDA_MASTER_PASS = os.getenv("PANDA_MASTER_PASS", "")
PANDA_MASTER_URL = os.getenv("PANDA_MASTER_URL", "https://pandamaster.vip/default.aspx")
...
def log(msg: str) -> None:
    ...
    print(f"[panda_master] {msg}", file=sys.stderr, flush=True)
...
def solve_captcha_via_telegram(image_bytes: bytes, request_id: str) -> str:
    ...
    caption = (
        f"🔢 CAPTCHA needed — Panda Master login\n"
...
def wait_for_topup_signal(player: str, amount: float) -> bool:
    ...
    alert = (
        f"⚠️ <b>AGENT FUNDS NEEDED</b> | Panda Master\n"
        f"Agent: <code>{PANDA_MASTER_USER}</code>\n"
    ...
    while time.time() - start < TOPUP_TIMEOUT_SEC:
        ...
        for upd in updates.get("result", []):
            ...
            if "loaded" in text:
                ...
                telegram_send_message(TOPUP_ALERT_GROUP_ID, f"✅ Top-up confirmed — retrying recharge for <code>{player}</code> (agent: <code>{PANDA_MASTER_USER}</code>)")
                return True
...
def login_panda_master(page: Page) -> bool:
    ...
    log(f"Navigating to {PANDA_MASTER_URL}")
    page.goto(PANDA_MASTER_URL, wait_until="domcontentloaded", timeout=45000)
    page.wait_for_timeout(2000)
    if is_dashboard_visible(page):
    ...
    for attempt in range(1, LOGIN_MAX_RETRIES + 1):
        ...
        try:
            ...
            try:
                ...
                pass
            page.locator("input#txtLoginName").fill(PANDA_MASTER_USER)
            page.locator("input#txtLoginPass").fill(PANDA_MASTER_PASS)

            # Screenshot just the CAPTCHA image element
            captcha_bytes = page.locator("img#ImageCheck").screenshot()
            request_id = f"pm_{int(time.time())}"
...
def _do_action_with_amount(
    ...
    try:
        ...
        # Panda Master: #Button1 has display:none. The visible submit is an <a> postback link inside the dialog.
    ...
    for attempt in range(1, TOPUP_MAX_RETRIES + 1):
        try:
            ...
            if attempt >= TOPUP_MAX_RETRIES:
                raise RuntimeError(
                    f"AGENT_FUNDS_EXHAUSTED: agent {PANDA_MASTER_USER} still insufficient after {TOPUP_MAX_RETRIES} top-up requests for player {account} amount ${amount:.2f}"
                )
...
def create_player(page: Page, account: str, password: str) -> dict:
    ...
    popup_text = ""
    try:
        popup_text, ctx = wait_for_success_popup(page, timeout_ms=5000)
        dismiss_popup(ctx)
...
def validate_env() -> list[str]:
    missing = []
    for key in ("ADSPOWER_API_KEY", "PANDA_MASTER_USER", "PANDA_MASTER_PASS", "TELEGRAM_BOT_TOKEN"):
    ...
    try:
        with sync_playwright() as pw:
            ...
            for _p in context.pages:
                ...
                if "pandamaster" in _url:
                    page = _p
                    break
            if page is None:
                page = context.new_page()

            if not login_panda_master(page):
