"""
Gameroom automation - controls AdsPower-protected Chrome via Playwright.

Supports login, balance, recharge, redeem, create, and reset for Gameroom.
"""
...
ADSPOWER_PROFILE_ID = os.getenv("ADSPOWER_PROFILE_ID", "k1clailr")
...
GAMEROOM_USER = os.getenv("GAMEROOM_USER", "")
GAMEROOM_PASS = os.getenv("GAMEROOM_PASS", "")
GAMEROOM_LOGIN_URL = os.getenv(
    "GAMEROOM_LOGIN_URL", "https://agentserver.gameroom777.com/admin/login"
)
GAMEROOM_DASHBOARD_URL = os.getenv(
    "GAMEROOM_DASHBOARD_URL", "https://agentserver.gameroom777.com/admin"
)
...
def log(msg: str) -> None:
    ...
    print(f"[gameroom] {msg}", file=sys.stderr, flush=True)
...
def telegram_send_photo(chat_id: str, photo_bytes: bytes, caption: str) -> dict | None:
    ...
    files = {"photo": ("gameroom_captcha.png", photo_bytes, "image/png")}
...
def solve_canvas_captcha(page: Page) -> str:
    ...
    temp_path = None
    with tempfile.NamedTemporaryFile(prefix="gameroom_captcha_", suffix=".png", delete=False) as tmp:
        tmp.write(image_bytes)
        temp_path = tmp.name

    telegram_send_photo(CAPTCHA_GROUP_ID, image_bytes, "Gameroom login")
...
def reload_dashboard_ready(page: Page) -> None:
    page.goto(GAMEROOM_DASHBOARD_URL)
    ...
    print("[gameroom] Dashboard reloaded and ready", file=sys.stderr, flush=True)


def login_gameroom(page: Page) -> bool:
    """Navigate to Gameroom login, solve canvas CAPTCHA, and reach dashboard."""
    if is_dashboard_url(page.url):
    ...
    log(f"Navigating to {GAMEROOM_LOGIN_URL}")
    page.goto(GAMEROOM_LOGIN_URL, wait_until="domcontentloaded", timeout=45000)
    page.wait_for_timeout(1000)
    ...
    for attempt in range(1, LOGIN_MAX_RETRIES + 1):
        ...
        dismiss_layui_popups(page)
        page.locator('input[name="username"]').fill(GAMEROOM_USER)
        page.locator('input[name="password"]').fill(GAMEROOM_PASS)
        ...
        print(f"[gameroom] post-submit URL: {page.url}", file=sys.stderr, flush=True)
...
def current_action_message(page: Page) -> str:
    ...
    /recharge, /withdraw, /resetpw). Validation popups from gameroom often appear
...
def wait_for_action_success(page: Page, timeout_ms: int = 12000) -> str:
    ...
    receipt_re = re.compile(r"GAMEROOM.*(DEPOSIT|WITHDRAW|TOTAL\s*BALANCE)", re.IGNORECASE | re.DOTALL)
    deadline = time.time() + timeout_ms / 1000
    last_message = ""
    ...
    # treat dialog closure as implicit success (gameroom often closes the dialog
...
def wait_for_topup_signal(player: str, amount: float) -> bool:
    ...
    alert = (
        f"AGENT FUNDS NEEDED | Gameroom\n"
        f"Agent: {GAMEROOM_USER}\n"
    ...
    for attempt in range(1, TOPUP_MAX_RETRIES + 1):
        ...
        if agent_balance_before is not None and amount > agent_balance_before:
            ...
            if attempt >= TOPUP_MAX_RETRIES:
                raise RuntimeError(
                    f"AGENT_FUNDS_EXHAUSTED: agent {GAMEROOM_USER} still insufficient after {TOPUP_MAX_RETRIES} top-up requests for player {account} amount ${amount:.2f}"
                )
            if not wait_for_topup_signal(account, amount):
                raise RuntimeError("Timed out waiting for 'loaded' reply in Telegram")

            # After a long Telegram wait (up to TOPUP_TIMEOUT_SEC = 30min), the gameroom
            # session has likely expired. Re-navigate to the dashboard and call
            # login_gameroom() — it's idempotent: returns immediately if session is still
            ...
            try:
                page.goto(GAMEROOM_DASHBOARD_URL, wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(1500)
            except Exception as e:
                log(f"Post-topup navigate ignored: {e}")
            if not login_gameroom(page):
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Gameroom automation")
...
def validate_env(action: str) -> list[str]:
    missing = []
    keys = ["ADSPOWER_API_KEY", "TELEGRAM_BOT_TOKEN", "GAMEROOM_USER", "GAMEROOM_PASS", "CAPTCHA_GROUP_ID"]
    for key in keys:
