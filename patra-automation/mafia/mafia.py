"""
Mafia automation - controls AdsPower-protected Chrome via Playwright.

Supports login, balance, recharge, redeem, create, and reset for Mafia.
"""
...
MAFIA_USER = os.getenv("MAFIA_USER", "")
MAFIA_PASS = os.getenv("MAFIA_PASS", "")
MAFIA_LOGIN_URL = os.getenv(
    "MAFIA_LOGIN_URL", "https://agentserver.mafia77777.com/admin/login"
)
MAFIA_DASHBOARD_URL = os.getenv(
    "MAFIA_DASHBOARD_URL", "https://agentserver.mafia77777.com/admin"
)
...
def log(msg: str) -> None:
    ...
    print(f"[mafia] {msg}", file=sys.stderr, flush=True)
...
def telegram_send_photo(chat_id: str, photo_bytes: bytes, caption: str) -> dict | None:
    ...
    files = {"photo": ("mafia_captcha.png", photo_bytes, "image/png")}
...
def solve_canvas_captcha(page: Page) -> str:
    ...
    temp_path = None
    with tempfile.NamedTemporaryFile(prefix="mafia_captcha_", suffix=".png", delete=False) as tmp:
        tmp.write(image_bytes)
        temp_path = tmp.name

    telegram_send_photo(CAPTCHA_GROUP_ID, image_bytes, "Mafia login")
...
def reload_dashboard_ready(page: Page) -> None:
    ...
    print("[mafia] Dashboard reloaded and ready", file=sys.stderr, flush=True)


def login_mafia(page: Page) -> bool:
    """Navigate to Mafia login, solve canvas CAPTCHA, and reach dashboard."""
    if is_dashboard_url(page.url):
    ...
    log(f"Navigating to {MAFIA_LOGIN_URL}")
    page.goto(MAFIA_LOGIN_URL, wait_until="domcontentloaded", timeout=45000)
    page.wait_for_timeout(1000)
    ...
    for attempt in range(1, LOGIN_MAX_RETRIES + 1):
        ...
        dismiss_layui_popups(page)
        page.locator('input[name="username"]').fill(MAFIA_USER)
        page.locator('input[name="password"]').fill(MAFIA_PASS)
        ...
        print(f"[mafia] post-submit URL: {page.url}", file=sys.stderr, flush=True)
...
def current_action_message(page: Page) -> str:
    ...
    /recharge, /withdraw, /resetpw). Validation popups from mafia often appear
...
def wait_for_action_success(page: Page, timeout_ms: int = 12000) -> str:
    ...
    # treat dialog closure as implicit success (mafia often closes the dialog
    ...
    for attempt in range(1, TOPUP_MAX_RETRIES + 1):
        ...
        if agent_balance_before is not None and amount > agent_balance_before:
            ...
            if attempt >= TOPUP_MAX_RETRIES:
                raise RuntimeError(
                    f"AGENT_FUNDS_EXHAUSTED: agent {MAFIA_USER} still insufficient after {TOPUP_MAX_RETRIES} top-up requests for player {account} amount ${amount:.2f}"
                )
            if not wait_for_topup_signal(account, amount):
                raise RuntimeError("Timed out waiting for 'loaded' reply in Telegram")

            # After a long Telegram wait (up to TOPUP_TIMEOUT_SEC = 30min), the mafia
            # session has likely expired. Re-navigate to the dashboard and call
            # login_mafia() — it's idempotent: returns immediately if session is still
            ...
            try:
                page.goto(MAFIA_DASHBOARD_URL, wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(1500)
            except Exception as e:
                log(f"Post-topup navigate ignored: {e}")
            if not login_mafia(page):
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Mafia automation")
...
def validate_env(action: str) -> list[str]:
    missing = []
    keys = ["ADSPOWER_API_KEY", "ADSPOWER_PROFILE_ID", "TELEGRAM_BOT_TOKEN", "MAFIA_USER", "MAFIA_PASS", "CAPTCHA_GROUP_ID"]
    for key in keys:
