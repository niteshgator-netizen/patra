"""
Fire Kirin automation — controls AdsPower-protected Chrome via Playwright.
...
FIRE_KIRIN_USER = os.getenv("FIRE_KIRIN_USER", "")
FIRE_KIRIN_PASS = os.getenv("FIRE_KIRIN_PASS", "")
FIRE_KIRIN_URL = os.getenv("FIRE_KIRIN_URL", "https://firekirin.xyz:8888/default.aspx")
...
def log(msg: str) -> None:
    ...
    print(f"[fire_kirin] {msg}", file=sys.stderr, flush=True)
...
def solve_captcha_via_telegram(image_bytes: bytes, request_id: str) -> str:
    ...
    caption = (
        f"🔢 CAPTCHA needed — Fire Kirin login\n"
...
def wait_for_topup_signal(player: str, amount: float) -> bool:
    ...
    alert = (
        f"⚠️ <b>AGENT FUNDS NEEDED</b> | Fire Kirin\n"
        f"Agent: <code>{FIRE_KIRIN_USER}</code>\n"
    ...
    while time.time() - start < TOPUP_TIMEOUT_SEC:
        ...
        for upd in updates.get("result", []):
            ...
            if "loaded" in text:
                ...
                telegram_send_message(TOPUP_ALERT_GROUP_ID, f"✅ Top-up confirmed — retrying recharge for <code>{player}</code> (agent: <code>{FIRE_KIRIN_USER}</code>)")
                return True
...
def login_fire_kirin(page: Page) -> bool:
    ...
    log(f"Navigating to {FIRE_KIRIN_URL}")
    page.goto(FIRE_KIRIN_URL, wait_until="domcontentloaded", timeout=45000)
    page.wait_for_timeout(2000)
    if is_dashboard_visible(page):
    ...
    for attempt in range(1, LOGIN_MAX_RETRIES + 1):
        ...
        try:
            ...
            try:
                ...
                pass
            page.locator("input#txtLoginName").fill(FIRE_KIRIN_USER)
            page.locator("input#txtLoginPass").fill(FIRE_KIRIN_PASS)

            # Screenshot just the CAPTCHA image element
            captcha_bytes = page.locator("img#ImageCheck").screenshot()
            request_id = f"fk_{int(time.time())}"
    ...
    for attempt in range(1, TOPUP_MAX_RETRIES + 1):
        try:
            ...
            if attempt >= TOPUP_MAX_RETRIES:
                raise RuntimeError(
                    f"AGENT_FUNDS_EXHAUSTED: agent {FIRE_KIRIN_USER} still insufficient after {TOPUP_MAX_RETRIES} top-up requests for player {account} amount ${amount:.2f}"
                )
...
def validate_env() -> list[str]:
    missing = []
    for key in ("ADSPOWER_API_KEY", "FIRE_KIRIN_USER", "FIRE_KIRIN_PASS", "TELEGRAM_BOT_TOKEN"):
    ...
    try:
        with sync_playwright() as pw:
            ...
            for _p in context.pages:
                ...
                if "firekirin" in _url:
                    page = _p
                    break
            if page is None:
                page = context.new_page()

            if not login_fire_kirin(page):
