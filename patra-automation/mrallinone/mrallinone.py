"""
Mr All In One automation - controls AdsPower-protected Chrome via Playwright.

Supports login, balance, recharge, redeem, create, and reset for Mr All In One.
"""
...
MRALLINONE_USER = os.getenv("MRALLINONE_USER", "")
MRALLINONE_PASS = os.getenv("MRALLINONE_PASS", "")
MRALLINONE_LOGIN_URL = os.getenv(
    "MRALLINONE_LOGIN_URL", "https://agentserver.mrallinone777.com/admin/login"
)
MRALLINONE_DASHBOARD_URL = os.getenv(
    "MRALLINONE_DASHBOARD_URL", "https://agentserver.mrallinone777.com/admin"
)
...
def log(msg: str) -> None:
    ...
    print(f"[mrallinone] {msg}", file=sys.stderr, flush=True)
...
def telegram_send_photo(chat_id: str, photo_bytes: bytes, caption: str) -> dict | None:
    ...
    files = {"photo": ("mrallinone_captcha.png", photo_bytes, "image/png")}
...
def solve_canvas_captcha(page: Page) -> str:
    ...
    temp_path = None
    with tempfile.NamedTemporaryFile(prefix="mrallinone_captcha_", suffix=".png", delete=False) as tmp:
        tmp.write(image_bytes)
        temp_path = tmp.name

    telegram_send_photo(CAPTCHA_GROUP_ID, image_bytes, "Mr All In One login")
...
def reload_dashboard_ready(page: Page) -> None:
    page.goto(MRALLINONE_DASHBOARD_URL)
    ...
    print("[mrallinone] Dashboard reloaded and ready", file=sys.stderr, flush=True)


def login_mrallinone(page: Page) -> bool:
    """Navigate to Mr All In One login, solve canvas CAPTCHA, and reach dashboard."""
    if is_dashboard_url(page.url):
    ...
    log(f"Navigating to {MRALLINONE_LOGIN_URL}")
    page.goto(MRALLINONE_LOGIN_URL, wait_until="domcontentloaded", timeout=45000)
    page.wait_for_timeout(1000)
    ...
    for attempt in range(1, LOGIN_MAX_RETRIES + 1):
        ...
        dismiss_layui_popups(page)
        page.locator('input[name="username"]').fill(MRALLINONE_USER)
        page.locator('input[name="password"]').fill(MRALLINONE_PASS)
        ...
        print(f"[mrallinone] post-submit URL: {page.url}", file=sys.stderr, flush=True)
...
def current_action_message(page: Page) -> str:
    ...
    /recharge, /withdraw, /resetpw). Validation popups from mrallinone often appear
...
def wait_for_action_success(page: Page, timeout_ms: int = 12000) -> str:
    ...
    # treat dialog closure as implicit success (mrallinone often closes the dialog
...
def wait_for_topup_signal(player: str, amount: float) -> bool:
    ...
    alert = (
        f"AGENT FUNDS NEEDED | Mr All In One\n"
        f"Agent: {MRALLINONE_USER}\n"
    ...
    for attempt in range(1, TOPUP_MAX_RETRIES + 1):
        ...
        if agent_balance_before is not None and amount > agent_balance_before:
            ...
            if attempt >= TOPUP_MAX_RETRIES:
                raise RuntimeError(
                    f"AGENT_FUNDS_EXHAUSTED: agent {MRALLINONE_USER} still insufficient after {TOPUP_MAX_RETRIES} top-up requests for player {account} amount ${amount:.2f}"
                )
            if not wait_for_topup_signal(account, amount):
                raise RuntimeError("Timed out waiting for 'loaded' reply in Telegram")

            # After a long Telegram wait (up to TOPUP_TIMEOUT_SEC = 30min), the mrallinone
            # session has likely expired. Re-navigate to the dashboard and call
            # login_mrallinone() — it's idempotent: returns immediately if session is still
            ...
            try:
                page.goto(MRALLINONE_DASHBOARD_URL, wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(1500)
            except Exception as e:
                log(f"Post-topup navigate ignored: {e}")
            if not login_mrallinone(page):
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Mr All In One automation")
...
def validate_env(action: str) -> list[str]:
    missing = []
    keys = ["ADSPOWER_API_KEY", "ADSPOWER_PROFILE_ID", "TELEGRAM_BOT_TOKEN", "MRALLINONE_USER", "MRALLINONE_PASS", "CAPTCHA_GROUP_ID"]
    for key in keys:
