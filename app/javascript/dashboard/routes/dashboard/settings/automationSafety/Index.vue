  ...
  <div class="pat-tpage flex-1 overflow-auto p-6">
    <div class="max-w-2xl">
      ...
      <p class="text-sm text-slate-400 mb-6">
        Control game transfers, player win-back, and fraud guards.
      </p>
      ...
      <div v-else class="space-y-8">
        ...
        <section class="space-y-4">
          ...
          <label class="block">
            ...
            <span class="text-xs text-slate-500"
              >When a player's winnings are below the cashout limit and they
              switch games.</span
            >
          </label>
          ...
          <label class="block">
            ...
            <select
              v-model="pref.transfer_deposit_shortfall_mode"
              class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm"
            >
              <option value="transfer_available">
                Transfer what they have
              </option>
              <option value="refuse">Refuse &amp; flag</option>
            </select>
            <span class="text-xs text-slate-500"
              >If their current balance is less than their deposit (only
              relevant for "deposit only").</span
            >
          </label>
        </section>
        ...
        <section class="space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <label class="block">
              ...
              <input
                v-model.number="pref.winback_dormant_days_vip"
                type="number"
                min="1"
                class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm"
              />
            ...
            <label class="block">
              ...
              <input
                v-model.number="pref.winback_dormant_days_regular"
                type="number"
                min="1"
                class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm"
              />
            ...
            <label class="block">
              ...
              <input
                v-model.number="pref.winback_dormant_days_new"
                type="number"
                min="1"
                class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm"
              />
        ...
        <section class="space-y-4">
          ...
          <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label class="block">
                ...
                <input
                  v-model.number="pref.fraud_cashout_velocity_count"
                  type="number"
                  min="1"
                  class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm"
                />
              ...
              <label class="block">
                ...
                <input
                  v-model.number="pref.fraud_cashout_velocity_hours"
                  type="number"
                  min="1"
                  class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm"
                />
          ...
          <label class="flex items-center gap-2">
            <input
              v-model="pref.fraud_duplicate_payment_check"
              type="checkbox"
            />
            <span class="text-sm">Duplicate-payment check</span>
          </label>
          <span class="text-xs text-slate-500 block -mt-2"
            >Block double-loading the same amount within 10 minutes.</span
          >
        </section>
        ...
        <section class="space-y-4">
          ...
          <label class="block">
            ...
            <select
              v-model="pref.payment_reply_source"
              class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm"
            >
