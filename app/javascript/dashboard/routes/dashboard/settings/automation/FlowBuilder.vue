...
const runPreview = async () => {
  ...
  if (!contactId) return;
  const { data } = await AutomationFlowsAPI.preview(route.params.flowId, {
    ...
    <div class="pat-page-main">
      <div class="flex h-full">
        ...
        <main class="flex flex-col flex-1 p-4">
          ...
          <div
            class="flex flex-wrap gap-3 min-h-[200px] p-4 rounded-xl bg-n-alpha-1"
          >
            <div
            ...
            >
              <p class="text-xs font-medium uppercase text-n-brand">
                {{ step.type }}
              </p>
          ...
          <div
            v-if="previewLog.length"
            class="p-3 mt-4 text-xs rounded-lg bg-n-alpha-1"
          >
            <h4 class="mb-2 font-medium">
              {{ $t('PATRA.FLOWS.PREVIEW_LOG') }}
            </h4>
