      ...
      <div class="border border-slate-700 rounded-lg p-4 mb-8">
        ...
        <div v-else class="space-y-6">
          <div class="grid grid-cols-2 gap-4">
            ...
            <label class="flex items-center gap-2">
              <input v-model="settings.referral_require_deposit" type="checkbox" />
              <span class="text-sm">Require deposit before payout</span>
            </label>
            <label class="block">
              <span class="text-xs text-slate-400">Referrer bonus ($)</span>
              <input v-model.number="settings.referral_bonus_referrer" type="number" step="0.01" class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm" />
            </label>
            <label class="block">
              <span class="text-xs text-slate-400">New player bonus ($)</span>
              <input v-model.number="settings.referral_bonus_new_player" type="number" step="0.01" class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm" />
            </label>
            <label class="block">
              <span class="text-xs text-slate-400">Bonus type</span>
              <select v-model="settings.referral_bonus_type" class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm">
            ...
            <label class="block">
              ...
              <select v-model="settings.referral_tracking_method" class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm">
            ...
            <label class="block">
              ...
              <input v-model="settings.referral_message_referrer" type="text" class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm" />
            </label>
            <label class="block">
              <span class="text-xs text-slate-400">New player message</span>
              <input v-model="settings.referral_message_new_player" type="text" class="w-full mt-1 p-2 bg-slate-800 border border-slate-600 rounded text-sm" />
            </label>
          </div>
...
export default {
  ...
  methods: {
    ...
    async saveSettings() {
      ...
      try {
        const accountId = this.$route.params.accountId;
        const { data } = await ReferralsAPI.updateReferralSettings(accountId, this.settings);
        this.settings = { ...this.settings, ...data };
