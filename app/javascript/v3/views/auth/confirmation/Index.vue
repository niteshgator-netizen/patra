...
<template>
  ...
  >
    ...
    >
      <div
        class="w-full max-w-[440px] relative bg-patra-surface/55 backdrop-blur-xl border border-patra-border-hi rounded-3xl p-10 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.5)] animate-card-in text-center"
      >
        <div
          class="w-[46px] h-[46px] rounded-[13px] bg-gradient-to-br from-patra to-patra-deep flex items-center justify-center font-display font-bold text-white text-2xl mb-5 animate-patra-pulse mx-auto"
        >
          {{ $t('PATRA_AUTH.BRAND_INITIAL') }}
        </div>
        <Spinner color-scheme="primary" size="" />
        <div class="ml-2 text-zinc-400 mt-4 text-sm">
          {{ $t('CONFIRM_EMAIL') }}
        </div>
      </div>
    </main>
