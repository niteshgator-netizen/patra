# Gameroom game client.
# Unlike Juwa/GameVault (REST APIs with MD5 signing), Gameroom has no public API.
...
module Games
  module Gameroom
    class Client
      ...
      class GameroomError < StandardError
      ...
      def test_connection
        ...
        call_listener('login', {})
        { ok: true, balance: nil, message: 'Gameroom VPS reachable; agent session valid' }
      rescue GameroomError => e
      ...
      def agent_balance
        # Gameroom's web panel does not expose agent balance via a clean script
      ...
      def user_balance(user_id:)
        # In this client, "user_id" is the player account (username). Gameroom
      ...
      def get_user_id(account_name:)
        # Gameroom has no numeric user_id. We pass through the username as the
      ...
      def recharge(user_id:, amount:, order_id:)
        ...
        # Gameroom panel accepts only whole-dollar amounts.
      ...
      def force_player_offline(user_id:)
        # Not supported via Gameroom panel. Return success-shaped stub so
        # callers that broadcast offline to all games don't crash.
        { 'code' => 0, 'msg' => 'not_supported_for_gameroom', 'data' => {} }
      end
      ...
      def call_listener(command, payload)
        uri = URI("#{@base_url}/gameroom/#{command}")
        ...
        Rails.logger.info("[Gameroom] POST #{uri} payload=#{safe_payload.to_json}")
        ...
        Rails.logger.info("[Gameroom] response status=#{response.code} body=#{body[0, 400]}")

        parsed =
          begin
            JSON.parse(body)
          rescue JSON::ParserError
            raise GameroomError.new(
        ...
        if parsed.is_a?(Hash) && parsed['status'] == 'error'
          ...
          raise GameroomError.new(
            em.presence || ec.presence || 'Unknown Gameroom error',
