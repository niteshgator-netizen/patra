# Mrallinone game client.
# Unlike Juwa/GameVault (REST APIs with MD5 signing), Mrallinone has no public API.
...
module Games
  module Mrallinone
    class Client
      ...
      class MrallinoneError < StandardError
      ...
      def test_connection
        ...
        call_listener('login', {})
        { ok: true, balance: nil, message: 'Mrallinone VPS reachable; agent session valid' }
      rescue MrallinoneError => e
      ...
      def agent_balance
        # Mrallinone's web panel does not expose agent balance via a clean script
      ...
      def user_balance(user_id:)
        # In this client, "user_id" is the player account (username). Mrallinone
      ...
      def get_user_id(account_name:)
        # Mrallinone has no numeric user_id. We pass through the username as the
      ...
      def recharge(user_id:, amount:, order_id:)
        ...
        # Mrallinone panel accepts only whole-dollar amounts.
      ...
      def force_player_offline(user_id:)
        # Not supported via Mrallinone panel. Return success-shaped stub so
        # callers that broadcast offline to all games don't crash.
        { 'code' => 0, 'msg' => 'not_supported_for_mrallinone', 'data' => {} }
      end
      ...
      def call_listener(command, payload)
        uri = URI("#{@base_url}/mrallinone/#{command}")
        ...
        Rails.logger.info("[Mrallinone] POST #{uri} payload=#{safe_payload.to_json}")
        ...
        Rails.logger.info("[Mrallinone] response status=#{response.code} body=#{body[0, 400]}")

        parsed =
          begin
            JSON.parse(body)
          rescue JSON::ParserError
            raise MrallinoneError.new(
        ...
        if parsed.is_a?(Hash) && parsed['status'] == 'error'
          ...
          raise MrallinoneError.new(
            em.presence || ec.presence || 'Unknown Mrallinone error',
