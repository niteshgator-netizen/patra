# Fire Kirin game client.
# Unlike Juwa/GameVault (REST APIs with MD5 signing), Fire Kirin has no public API.
...
module Games
  module FireKirin
    class Client
      ...
      class FireKirinError < StandardError
      ...
      def test_connection
        ...
        call_listener('login', {})
        { ok: true, balance: nil, message: 'Fire Kirin VPS reachable; agent session valid' }
      rescue FireKirinError => e
      ...
      def agent_balance
        # Fire Kirin's web panel does not expose agent balance via a clean script
      ...
      def user_balance(user_id:)
        # In this client, "user_id" is the player account (username). Fire Kirin
      ...
      def get_user_id(account_name:)
        # Fire Kirin has no numeric user_id. We pass through the username as the
      ...
      def recharge(user_id:, amount:, order_id:)
        ...
        # Fire Kirin panel accepts only whole-dollar amounts.
      ...
      def force_player_offline(user_id:)
        # Not supported via Fire Kirin panel. Return success-shaped stub so
        # callers that broadcast offline to all games don't crash.
        { 'code' => 0, 'msg' => 'not_supported_for_fire_kirin', 'data' => {} }
      end
      ...
      def call_listener(command, payload)
        uri = URI("#{@base_url}/fire_kirin/#{command}")
        ...
        Rails.logger.info("[FireKirin] POST #{uri} payload=#{safe_payload.to_json}")
        ...
        Rails.logger.info("[FireKirin] response status=#{response.code} body=#{body[0, 400]}")

        parsed =
          begin
            JSON.parse(body)
          rescue JSON::ParserError
            raise FireKirinError.new(
        ...
        if parsed.is_a?(Hash) && parsed['status'] == 'error'
          ...
          raise FireKirinError.new(
            em.presence || ec.presence || 'Unknown Fire Kirin error',
