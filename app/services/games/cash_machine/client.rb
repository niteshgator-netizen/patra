# Cash Machine game client.
# Unlike Juwa/GameVault (REST APIs with MD5 signing), Cash Machine has no public API.
...
module Games
  module CashMachine
    class Client
      ...
      class CashMachineError < StandardError
      ...
      def test_connection
        ...
        call_listener('login', {})
        { ok: true, balance: nil, message: 'Cash Machine VPS reachable; agent session valid' }
      rescue CashMachineError => e
      ...
      def agent_balance
        # Cash Machine's web panel does not expose agent balance via a clean script
      ...
      def user_balance(user_id:)
        # In this client, "user_id" is the player account (username). Cash Machine
      ...
      def get_user_id(account_name:)
        # Cash Machine has no numeric user_id. We pass through the username as the
      ...
      def recharge(user_id:, amount:, order_id:)
        ...
        # Cash Machine panel accepts only whole-dollar amounts.
      ...
      def force_player_offline(user_id:)
        # Not supported via Cash Machine panel. Return success-shaped stub so
        # callers that broadcast offline to all games don't crash.
        { 'code' => 0, 'msg' => 'not_supported_for_cash_machine', 'data' => {} }
      end
      ...
      def call_listener(command, payload)
        uri = URI("#{@base_url}/cash_machine/#{command}")
        ...
        Rails.logger.info("[CashMachine] POST #{uri} payload=#{safe_payload.to_json}")
        ...
        Rails.logger.info("[CashMachine] response status=#{response.code} body=#{body[0, 400]}")

        parsed =
          begin
            JSON.parse(body)
          rescue JSON::ParserError
            raise CashMachineError.new(
        ...
        if parsed.is_a?(Hash) && parsed['status'] == 'error'
          ...
          raise CashMachineError.new(
            em.presence || ec.presence || 'Unknown Cash Machine error',
