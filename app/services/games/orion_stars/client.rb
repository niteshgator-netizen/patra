# Orion Stars game client.
# Unlike Juwa/GameVault (REST APIs with MD5 signing), Orion Stars has no public API.
...
module Games
  module OrionStars
    class Client
      ...
      class OrionStarsError < StandardError
      ...
      def test_connection
        ...
        call_listener('login', {})
        { ok: true, balance: nil, message: 'Orion Stars VPS reachable; agent session valid' }
      rescue OrionStarsError => e
      ...
      def agent_balance
        # Orion Stars's web panel does not expose agent balance via a clean script
      ...
      def user_balance(user_id:)
        # In this client, "user_id" is the player account (username). Orion Stars
      ...
      def get_user_id(account_name:)
        # Orion Stars has no numeric user_id. We pass through the username as the
      ...
      def recharge(user_id:, amount:, order_id:)
        ...
        # Orion Stars panel accepts only whole-dollar amounts.
      ...
      def force_player_offline(user_id:)
        # Not supported via Orion Stars panel. Return success-shaped stub so
        # callers that broadcast offline to all games don't crash.
        { 'code' => 0, 'msg' => 'not_supported_for_orion_stars', 'data' => {} }
      end
      ...
      def call_listener(command, payload)
        uri = URI("#{@base_url}/orion_stars/#{command}")
        ...
        Rails.logger.info("[OrionStars] POST #{uri} payload=#{safe_payload.to_json}")
        ...
        Rails.logger.info("[OrionStars] response status=#{response.code} body=#{body[0, 400]}")

        parsed =
          begin
            JSON.parse(body)
          rescue JSON::ParserError
            raise OrionStarsError.new(
        ...
        if parsed.is_a?(Hash) && parsed['status'] == 'error'
          ...
          raise OrionStarsError.new(
            em.presence || ec.presence || 'Unknown Orion Stars error',
