# Milky Way game client.
# Unlike Juwa/GameVault (REST APIs with MD5 signing), Milky Way has no public API.
...
module Games
  module MilkyWay
    class Client
      ...
      class MilkyWayError < StandardError
      ...
      def test_connection
        ...
        call_listener('login', {})
        { ok: true, balance: nil, message: 'Milky Way VPS reachable; agent session valid' }
      rescue MilkyWayError => e
      ...
      def agent_balance
        # Milky Way's web panel does not expose agent balance via a clean script
      ...
      def user_balance(user_id:)
        # In this client, "user_id" is the player account (username). Milky Way
      ...
      def get_user_id(account_name:)
        # Milky Way has no numeric user_id. We pass through the username as the
      ...
      def recharge(user_id:, amount:, order_id:)
        ...
        # Milky Way panel accepts only whole-dollar amounts.
      ...
      def force_player_offline(user_id:)
        # Not supported via Milky Way panel. Return success-shaped stub so
        # callers that broadcast offline to all games don't crash.
        { 'code' => 0, 'msg' => 'not_supported_for_milky_way', 'data' => {} }
      end
      ...
      def call_listener(command, payload)
        uri = URI("#{@base_url}/milky_way/#{command}")
        ...
        Rails.logger.info("[MilkyWay] POST #{uri} payload=#{safe_payload.to_json}")
        ...
        Rails.logger.info("[MilkyWay] response status=#{response.code} body=#{body[0, 400]}")

        parsed =
          begin
            JSON.parse(body)
          rescue JSON::ParserError
            raise MilkyWayError.new(
        ...
        if parsed.is_a?(Hash) && parsed['status'] == 'error'
          ...
          raise MilkyWayError.new(
            em.presence || ec.presence || 'Unknown Milky Way error',
