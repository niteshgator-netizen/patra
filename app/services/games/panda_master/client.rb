# Panda Master game client.
# Unlike Juwa/GameVault (REST APIs with MD5 signing), Panda Master has no public API.
...
module Games
  module PandaMaster
    class Client
      ...
      class PandaMasterError < StandardError
      ...
      def test_connection
        ...
        call_listener('login', {})
        { ok: true, balance: nil, message: 'Panda Master VPS reachable; agent session valid' }
      rescue PandaMasterError => e
      ...
      def agent_balance
        # Panda Master's web panel does not expose agent balance via a clean script
      ...
      def user_balance(user_id:)
        # In this client, "user_id" is the player account (username). Panda Master
      ...
      def get_user_id(account_name:)
        # Panda Master has no numeric user_id. We pass through the username as the
      ...
      def recharge(user_id:, amount:, order_id:)
        ...
        # Panda Master panel accepts only whole-dollar amounts.
      ...
      def force_player_offline(user_id:)
        # Not supported via Panda Master panel. Return success-shaped stub so
        # callers that broadcast offline to all games don't crash.
        { 'code' => 0, 'msg' => 'not_supported_for_panda_master', 'data' => {} }
      end
      ...
      def call_listener(command, payload)
        uri = URI("#{@base_url}/panda_master/#{command}")
        ...
        Rails.logger.info("[PandaMaster] POST #{uri} payload=#{safe_payload.to_json}")
        ...
        Rails.logger.info("[PandaMaster] response status=#{response.code} body=#{body[0, 400]}")

        parsed =
          begin
            JSON.parse(body)
          rescue JSON::ParserError
            raise PandaMasterError.new(
        ...
        if parsed.is_a?(Hash) && parsed['status'] == 'error'
          ...
          raise PandaMasterError.new(
            em.presence || ec.presence || 'Unknown Panda Master error',
